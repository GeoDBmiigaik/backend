...
async def get_users_table(conn: AsyncConnection, current_user: str):
    ...
    async for table_info in users_tables:
        ...
        result = Result(
            columns=({'field': col, 'headerName': col, 'width': 150, 'editable': True}
                     for col in tuple(data.keys())))
        async for row in data:
            result.rows.append({key['field']: float(row._asdict()[key['field']]) if type(
                row._asdict()[key['field']]) == decimal.Decimal else row._asdict()[key['field']]
                                for key in result.columns})
        tables_list.append(result)
    return tables_list


async def get_users_tables_namelist(conn: AsyncConnection, current_user: str):
    SQL = """select cls.relname as object_name
                            from pg_class cls
                              join pg_roles rol on rol.oid = cls.relowner
                              join pg_namespace nsp on nsp.oid = cls.relnamespace
                            where nsp.nspname not in ('information_schema', 'pg_catalog', 'tiger', 'topology')
